...
import { useWavesurferContext } from 'components/WaveSurferContext'

interface Props extends RegionContentProps {
  deleteRegion?: (regionId: string) => Promise<boolean>
}
const CommonRegion = (props: Props): ReactElement => {
  ...
  const handleClick = async (e, type: string): Promise<void> => {
    switch (type) {
      case 'CREATE':
      case 'EDIT':
  ...
  const handleRemove = async (): Promise<void> => {
    ...
    console.log(props.region.id)

    if (props.region.id && !props.region.id.startsWith('wavesurfer')) {
      remove = (await props.deleteRegion?.(props.region.id)) ?? true
    }
