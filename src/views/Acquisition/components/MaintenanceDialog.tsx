...
import { maintenanceDialogMessages } from '../messages'
...
const MaintenanceDialog = (props: Props): ReactElement => {
  ...
  const toggleQuarantine = async (): Promise<void> => {
    try {
      const updated = await actions?.updateMaintenance(props.id, props.release)

      if (updated) {
        toast.success(
          formatMessage(maintenanceDialogMessages.success, {
      ...
      <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div className="text-center sm:mt-0">
          ...
          <h3 className="text-lg font-medium leading-6 text-gray-900 mb-3">
            {formatMessage(maintenanceDialogMessages.title, {
              status: props.release ? 'quarantine' : 'other'
            })}
          </h3>
          <p className="text-sm text-gray-500 mt-1">
            {formatMessage(maintenanceDialogMessages.question, {
              status: props.release ? 'quarantine' : 'other',
              phone: props.phone
            })}
          </p>
